...
@azapi.route("/azapi/customer/<customer_id>", methods=['GET'])
def getCustomerUsers(customer_id):

    try:

	select_customer_dn = db.session.query(TblCustomer.txt_customer_dn).filter(TblCustomer.uid_customer_id == customer_id).all()
...
def createUserLdap():
    try:
        ...
        select_customer_dn = db.session.query(TblCustomer.txt_customer_dn).filter(TblCustomer.uid_customer_id == customer_id).all()
        customer_dn=select_customer_dn[0][0]
        my_logger.debug(customer_dn), "thhhhhhhhhhhhhhhhhhhhhhhoooooooooooooooooorrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr"
        customer_gid_query = db.session.query(TblCustomer.int_gid_id).filter(TblCustomer.uid_customer_id==customer_id).all()
        customer_gid_id = customer_gid_query[0][0]
        my_logger.debug(customer_gid_id),"theeeeeeeeeeeeeeeeeeeeeeeeeeeeerrrrrrrrrrrrrrrrrrrrrrrrrreeeeeeeeeeeeeeeeeeeeeee"
	...
	my_logger.debug(addinguser)
        if addinguser:
            ...
            db.session.add(data)
            db.session.commit()
            print 'usssssssssssssssssssssssssssssseeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeerrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrsssssssssssssssssssssssssss'
            select_user_dn = db.session.query(TblUsers).filter(TblUsers.var_user_name == uid).order_by(TblUsers.srl_id.desc()).all()
...
def cluster(cusid,cluid):
    try:
        ...
        req_data=collection.find({"cluster_id": cluid,"time": {"$gte":from_time_params_str,"$lte":to_time_params_str}})
        payload_list=[]
...
@azapi.route('/cluster/<customer_id>',methods=['GET'])
def cluster_info(customer_id):
    conn = psycopg2.connect(conn_string)
    cur=conn.cursor()
    cur.execute("set search_path to highgear;")

    customer_cluster_query_stmnt="SELECT uid_customer_id,uid_cluster_id,uid_cluster_type_id,valid_cluster FROM tbl_cluster where uid_customer_id='"+str(customer_id)+"'"
    ...
    if customer_cluster_info==[]:
    	...
    	for cluster_info in customer_cluster_info:
            if cluster_info[3] == True:
                print cluster_info,'clus_info'
                customer_type_query="select char_name from tbl_cluster_type where uid_cluster_type_id='"+str(cluster_info[2])+"'"
                cur.execute(customer_type_query)
                clustername=cur.fetchall()
                print clustername[0][0].rstrip(),"clustname"
                clus_name = clustername[0][0].rstrip()
                #list_customer_cluster_info=[]
                node_info_stmnt="select uid_node_id,char_role,edge_node from tbl_node_information where uid_cluster_id='"+str(cluster_info[1])+"' and edge_node = true"
                cur.execute(node_info_stmnt)
                cus_node_info=cur.fetchall()
                print cus_node_info,"cusss"
                list_cus=[]

                for cus in cus_node_info:
                    list_cus.append({"node_id":cus[0],"char_role":cus[1]})
                #print list_cus,"listcuss"
                list_customer_cluster_info.append({"customer_id":cluster_info[0],"node_information":list_cus,"cluster_id":cluster_info[1],"cluster_type_id":cluster_info[2],"clustername":clus_name,"valid_cluster":cluster_info[3],"available_storage":available_storage})
                print list_customer_cluster_info, 'list_custoooooooooooooooo'
        reversed_list_customer_cluster_info= list_customer_cluster_info[::-1]
        print reversed_list_customer_cluster_info,"rev"
...
def clustermembers(customer_id,cluster_id):
    ...
    #Query cluster members from tbl_node_information
        ...
        cluster_info_query_statement=db.session.query(TblNodeInformation).\
            filter(TblNodeInformation.uid_customer_id==customer_id,TblNodeInformation.uid_cluster_id==cluster_id).all()
        print cluster_info_query_statement
        list_cluster_info=[i.to_json() for i in cluster_info_query_statement]
        print list_cluster_info,"cluster_info"
        cluster_roles=[]
...
def azureFileStorageCredentials(customer_id):
    ...
    try:
        ...
        #print azure_file_storage_credentials_statement,'aaaaaaaaaaaaaa'
        #print azure_file_storage_credentials_statement[0],'aaaaaaaaaaaaaaaaaaaaa'
        values_list = []
        keys_list = []
        for val in azure_file_storage_credentials_statement:
            ...
            #print azure_file_storage_credentials_statement_result,'resssssssssssssssssss'
        #print keys_list,'keyyyyyyyyyyyyyyyyyyyyyy'
