...
from application.models.models import TblCustomerJobRequest, TblMetaFileUpload, TblFileUpload,TblMetaCloudType,TblMetaVmSize,TblPlanClusterSizeConfig ,TblMetaMrRequestStatus, TblCustomerRequest, TblMetaRequestStatus
...
@mrapi.route("/api/mrjobstatus/<mr_job_id>",methods=['GET'])
...
@mrapi.route("/api/jobstatus/<job_id>",methods=['GET'])
def JobStatus(job_id):
    print "inside"
    session = scoped_session(session_factory)
    result_dict = {}
    customer_request_id_list = session.query(TblCustomerRequest.int_request_status)\
        .filter(TblCustomerRequest.uid_request_id == job_id).all()
    print customer_request_id_list
    for request_id in customer_request_id_list:
        request_id_status = session.query(TblMetaRequestStatus.var_request_status)\
            .filter(TblMetaRequestStatus.srl_id == request_id).all()
        print request_id_status
        result_dict[job_id] = request_id_status[0][0]
    print result_dict
    return jsonify(result_dict)
