...
def customercreation():
    ...
    db.session.query(TblCustomer).filter(TblCustomer.uid_customer_id == customer_id).update(
    ...
    db.session.query(TblAzureAppGateway).filter(TblAzureAppGateway.uid_gateway_id == gateway_id).update(
