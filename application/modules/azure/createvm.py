...
#            subnet_id = '/subscriptions/cfed96cf-1241-4c76-827e-785b6d5cff7c/resourceGroups/haas/providers/Microsoft.Network/virtualNetworks/haas/subnets/SUB1539665969989'
    ...
    -   content: |        
        ...
        path: /opt/agent/agent_info.txt""".format(customerid=customerid, clusterid=clusterid, role=role, agentid=agentid)
            ...
            # Create Linux VM
            my_logger.info('Creating Linux Virtual Machine')
            ...
            my_logger.info('Get Virtual Machine by Name')
            ...
            my_logger.info('os_profffffffffffffffilllle')
...
def create_nic_namenode(network_client, subnet_id, GROUP_NAME, IP_NAME, LOCATION, IP_CONFIG_NAME, NIC_NAME):
    ...
    # Create NIC
    my_logger.info('Create NIC with ')
   # my_logger.info(("Nic Name:", NIC_NAME, "Location:", LOCATION, "ipconfigName:", IP_CONFIG_NAME, "SubnetId", subnet_id,"publc ipid",public_ip_id)
...
def create_nic(network_client, subnet_id, GROUP_NAME, NIC_NAME, LOCATION, IP_CONFIG_NAME):
    my_logger.info('Create NIC')
    my_logger.info(GROUP_NAME)
    my_logger.info(NIC_NAME)
    my_logger.info(LOCATION)
    my_logger.info(IP_CONFIG_NAME)
    async_nic_creation = network_client.network_interfaces.create_or_update(
        GROUP_NAME,
        NIC_NAME,
        {'name': NIC_NAME,
...
def create_vm_parameters(encoded, nic_id, OS_DISK_NAME, LOCATION,image_id,vm_size):
    ...
    my_logger.info(image_id,'in  parameters')
    return {
        'location': LOCATION,
        'os_profile': {
