...
def configure_cluster(request_id):
    ...
    # generating task type info dict for task generation
        ...
        task_type_role_info = session.query(TblTaskType.char_task_type_id,TblTaskType.int_vm_roles).filter(TblTaskType.char_task_type_id.in_(task_type_list)).all()

        task_types_info_dict = {}
        for task_types in task_type_role_info:
            task_types_info_list = session.query(TblMetaNodeRoles.vm_roles).filter(TblMetaNodeRoles.srl_id == task_types[1]).all()
            task_types_info_dict[task_types[0]] = task_types_info_list[0][0]


    #data querying from vm creation table

        vm_roles_query=session.query(TblVmCreation.uid_cluster_id,TblVmCreation.uid_agent_id,TblVmCreation.var_role,TblVmCreation.var_ip,TblVmCreation.uid_vm_id).filter(TblVmCreation.uid_customer_id == customer_id,TblVmCreation.uid_cluster_id == cluster_id).all()

    # generating cluster_information_dictionary for task generation method
    ...
    # passing parameters to task generation

        task_generator = generate_tasks(dict_nodes=cluster_information_dict, dict_tasktypes=task_types_info_dict)



    #inserting into node_information table

        time_now=datetime.datetime.now()
        for vm_information in vm_roles_query:
            node_id=str(uuid.uuid1())
#            fqdn_string=str(vm_information[5]) + ".kwartile"
            node_information_insertion=TblNodeInformation(uid_node_id=node_id,uid_vm_id=vm_information[4],uid_cluster_id=cluster_id,uid_customer_id=customer_id,char_role=vm_information[2],var_created_by='system',var_modified_by='system',ts_created_datetime=time_now,ts_modified_datetime=time_now)
            session.add(node_information_insertion)
            session.commit()

    #inserting into agent table

        node_information_query=session.query(TblNodeInformation.uid_node_id,TblNodeInformation.uid_vm_id).filter(TblNodeInformation.uid_customer_id == customer_id,TblNodeInformation.uid_cluster_id == cluster_id).all()
        for vm_creation_information in vm_roles_query:
            for node_information_individual in node_information_query:
                if vm_creation_information[4] == node_information_individual[1]:
                    agent_insertion=TblAgent(uid_agent_id=vm_creation_information[1],uid_node_id=node_information_individual[0],uid_cluster_id=cluster_id,uid_customer_id=customer_id,private_ips=vm_creation_information[3],var_created_by='system',var_modified_by='system',ts_created_datetime=time_now,ts_modified_datetime=time_now)
    ...
    #inserting into kafka tables
        ...
        for individualtopic in topiclist:
            ...
            kafka_topic_name_insertion = TblKafkaTopic(uid_topic_id=topicid, var_topic_name=topicname,var_topic_type=individualtopic,var_created_by='system',var_modified_by='system',ts_created_datetime=time_now,ts_modified_datetime=time_now)
            session.add(kafka_topic_name_insertion)
            session.commit()
            kafka_topic_id_insertion = TblKafkaPublisher(uid_topic_id=topicid, uid_cluster_id=cluster_id,uid_customer_id=customer_id,var_created_by='system',var_modified_by='system',ts_created_datetime=time_now,ts_modified_datetime=time_now)
            session.add(kafka_topic_id_insertion)
            session.commit()

    #declairing  strings for slaves,host,configuration to append ips
    ...
    # Appending slaves string object id into list
    ...
    # generating list dependent task dict to give input for dependency task method
        ...
        dependency_task_types_info = session.query(TblTaskType.char_task_type_id,TblTaskType.txt_dependency_task_id).filter(TblTaskType.char_task_type_id.in_(task_type_list)).all()
        ...
        dependent_task_info = find_dep_tasks(dict_tasktypes=list_dependent_tasks_dict,dict_tasks=task_tasktype_mapping_dict)
    ...
    #adding worker path for each task in list
        ...
        task_path = session.query(TblTaskType.char_task_type_id,TblTaskType.txt_agent_worker_version_path).filter(TblTaskType.char_task_type_id.in_(task_type_list)).all()
        for task_information in task_generator:
            for task_type_information in task_path:
                if task_information[2] == task_type_information[0]:
                    task_information.append(task_path[0][1])
            ...
            if len(task_information) == 7:
                if task_information[5] == '':
                    task_insertion = TblTask(uid_task_id=str(task_information[1]),
                                             ...
                                             txt_agent_worker_version_path=task_information[6],var_created_by='system',var_modified_by='system',ts_created_datetime=time_now,ts_modified_datetime=time_now)
                else:
                    task_insertion = TblTask(uid_task_id=str(task_information[1]),
                                             char_task_type_id=task_information[2], int_task_status=task_status_value,
                                             uid_request_id=str(request_id), char_feature_id=feature_id,
                ...
                if task_information[4] == '':
                    task_insertion = TblTask(uid_task_id=str(task_information[1]),
                                             ...
                                             txt_agent_worker_version_path=task_information[5],var_created_by='system',var_modified_by='system',ts_created_datetime=time_now,ts_modified_datetime=time_now)
                else:
                    task_insertion = TblTask(uid_task_id=str(task_information[1]),
                                             char_task_type_id=task_information[2], int_task_status=task_status_value,
                                             uid_request_id=str(request_id), char_feature_id=feature_id,
