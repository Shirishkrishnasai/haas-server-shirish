...
def serverworker(request_id):
    ...
    customerdata = "select txt_payload_id,lng_customer_id,lng_feature_id from tbl_customer_request where uid_request_id=" + str(
        request_id)
    print(request_id)
    ...
    cluster_statement = "insert into tbl_cluster (uid_cluster_id,lng_customer_id) values ('" + str(
    ...
    for values in querystatment.values():
        ...
        if value != payloadid:
            ...
            # slave.write("%s\n" % vm_ip)
            host.write("%s %s\n" % (vm_ip, vm_fqdn))
            vm_insert = "insert into tbl_vm_information (uid_vm_id,lng_customer_id,var_group_name,var_virtual_network_name,lng_subnet_id) values ('" + str(
                vm_id) + "','" + str(customer_id) + "','" + str(resource_group_id) + "','" + str(
                network_id) + "'," + str(node_id) + ")"
            print vm_insert
            ...
            node_statement = "insert into tbl_node_information (uid_vm_id,lng_customer_id,uid_cluster_id,txt_fqdn,char_role) values ('" + str(
                vm_id) + "'," + str(customer_id) + ",'" + str(cluster_id) + "','" + str(vm_fqdn) + "'" + ",'" + str(
                values["role"]) + "')"
            ...
            insertstatement = "insert into tbl_agent (lng_node_id,uid_agent_id,uid_cluster_id,lng_customer_id,ts_registered_datetime) values (" + str(
                srl_node_id) + "," + "'" + str(agentid) + "'" + ",'" + str(cluster_id) + "'," + str(
            ...
            if values["role"] == "namenode":
                ...
                statement = "select lng_task_type_id,txt_agent_worker_version_path,txt_agent_worker_version from tbl_task_types where lng_task_type_id in " + str(
                ...
                for tasks in namenode_tasks:
                    ...
                    insert = "insert into tbl_tasks (lng_task_type_id,uid_task_id,uid_request_id,lng_feature_id,uid_agent_id,lng_customer_id,ts_assigned_time,txt_agent_worker_version_path,txt_agent_worker_version) values (" + str(
                        task_type_id) + ",'" + str(task_id) + "'," + str(request_id) + "," + str(
                        feature_id) + ",'" + str(agentid) + "'," + str(customer_id) + "," + "'" + str(
                        assigned_time) + "','" + str(worker_version_path) + "','" + str(worker_version) + "')"
                    cur.execute(insert)
                    conn.commit()
    ...
    node_task_idsquery = "select uid_task_id from tbl_tasks where lng_task_type_id in (4,11)"
    ...
    dependent_tasks_list = [[1, []], [2, []], [3, []], [4, []], [11, []], [5, [1, 2, 3, 11, 4, 6, 7, 8, 9]],
                            [6, [1, 2, 3, 11, 4, 7, 8, 9]], [7, [8, 9, 3, 11]], [8, [3, 9, 11]], [9, [3, 11]],
    ...
    for dependenttasks in dependent_tasks_list:
        if dependenttasks[1] == []:
            ...
            if len(dependenttasks[1]) == 1:
                ...
                taskidliststatement = "select uid_task_id from tbl_tasks where lng_task_type_id = " + str(
                ...
                taskidliststatement = "select uid_task_id from tbl_tasks where lng_task_type_id in " + str(
            ...
            updatedependenttasks = "update tbl_tasks set txt_dependent_task_id =" + str(
                tasksstring) + " where lng_task_type_id=" + str(dependenttasks[0])
    ...
    cur = conn.cursor()
    cur.execute("set search_path to highgear;")
    update_tasks_status = "update tbl_tasks set var_task_status='created' where var_task_status='C'"
    cur.execute(update_tasks_status)
    conn.commit()
